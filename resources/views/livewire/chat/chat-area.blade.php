...
<div class="flex flex-col h-full" style="overflow-x:hidden; max-width:100vw;"
     wire:poll.120s.visible
     x-data="chatArea()"
     x-init="init()"
     @scroll-to-bottom.window="scrollToBottom(true)"
     @scroll-to-message.window="$nextTick(() => { setTimeout(() => { const el = document.getElementById('msg-' + $event.detail.id); if(el) el.scrollIntoView({behavior:'instant', block:'start'}); }, 50); })"
...
>
    ...
    <div class="chat-header" style="min-height:56px; border-bottom:1px solid rgba(255,255,255,0.05); display:flex; align-items:center; padding:8px 12px; gap:8px; flex-shrink:0; background:rgba(11,15,28,0.6); backdrop-filter:blur(8px); overflow:hidden;">
        ...
        <div style="position:relative; flex-shrink:0;" x-data="{ showAvatar: false }">
                ...
                <div wire:key="msg-{{ $msg->id }}" id="msg-{{ $msg->id }}" style="display:flex; align-items:flex-end; gap:8px; max-width:75%; position:relative;{{ $msgSelectMode && in_array($msg->id, $forwardMessageIds) ? ' background:rgba(59,130,246,0.08); border-radius:8px; padding:4px;' : '' }}" x-data="{ showMenu: false }" @mouseenter="if(window.matchMedia('(hover:hover)').matches) showMenu = true" @mouseleave="if(window.matchMedia('(hover:hover)').matches) showMenu = false" @click="if(!window.matchMedia('(hover:hover)').matches && showMenu === false) showMenu = true" @click.outside="showMenu = false">
                    ...
                    <div>
                        ...
                        @elseif($msg->type === 'video')
                            <div style="background:rgba(31,41,55,0.8); border-radius:18px 18px 18px 4px; overflow:hidden; border:1px solid rgba(255,255,255,0.06); max-width:min(300px, 70vw);">
                                @if($msg->sender_name)
                                    <p style="font-size:11px; font-weight:700; color:{{ senderColor($msg->sender_phone ?? $msg->sender_name) }}; padding:8px 10px 4px;">{{ $msg->sender_name }}</p>
                                @endif
    ...
    <div style="border-top:1px solid rgba(255,255,255,0.05); padding:10px 12px; flex-shrink:0; background:rgba(8,12,22,0.7); backdrop-filter:blur(8px); position:relative;"
        ...
        <div x-show="showEmoji"
            ...
            <div style="display:grid; grid-template-columns:repeat(9, 1fr); gap:2px; max-height:200px; overflow-y:auto;">
                @foreach(['😀','😁','😂','🤣','😃','😄','😅','😆','😉','😊','😋','😎','😍','🥰','😘','🤗','🤩','😐','🙄','😏','😔','😴','😷','🤒','🤧','🥵','🤯','😈','💀','💩','👻','🤖','💪','👋','👌','✌','🤞','👍','👎','✊','👏','🙌','🤝','🙏','❤','🧡','💛','💚','💙','💜','🖤','💔','💯','🔥','✨','⭐','🎉','🎊','🎈','🎁','🏆','🥇','👀','💡','💬','📱','💻','⏰','📅','✅','❌','⚠️','🔔','📢','💰','💳','🛒','🚀','✈️','🌍','🏠','🚗','🍕','🍔','🍟','🌮','🍜','🍣','🍦','🎂','🍺','🍻','☕','🧃','🌹','🌺','🌸','🌻','🐶','🐱','🐭','🐹','🐰','🦊','🐻','🐼','🐨','🐯','🦁','🐮','🐷','🐸','🐵','🦋','🌈','⛅','🌙','⚡','🌊'] as $emoji)
                    <button type="button"
                            @click.stop="const el=document.getElementById('main-message-input'); el.value += '{{ $emoji }}'; el.focus(); showEmoji=false;"
        ...
        <div style="display:flex; align-items:flex-end; gap:8px;">
            ...
            {{-- Send button (always green, sends text or stops recording) --}}
