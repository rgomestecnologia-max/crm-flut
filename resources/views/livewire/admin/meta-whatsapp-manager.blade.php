...
<div style="max-width:900px; margin:0 auto; padding:24px 16px; display:flex; flex-direction:column; gap:16px;">
    ...
    @if(session('success'))
    <div style="background:rgba(34,197,94,0.12); border:1px solid rgba(34,197,94,0.3); border-radius:10px; padding:12px 16px; color:#4ade80; font-size:13px; font-weight:600;">
        {{ session('success') }}
    </div>
    @endif
    @if(session('error'))
    <div style="background:rgba(239,68,68,0.12); border:1px solid rgba(239,68,68,0.3); border-radius:10px; padding:12px 16px; color:#f87171; font-size:13px; font-weight:600;">
        {{ session('error') }}
    </div>
    @endif
    ...
    <div style="background:linear-gradient(145deg, rgba(17,24,39,0.9), rgba(11,15,28,0.95)); border:1px solid rgba(24,119,242,0.2); border-radius:16px; padding:24px; position:relative; overflow:hidden;">
        ...
        <div style="display:flex; align-items:center; gap:16px; flex-wrap:wrap;">
            ...
            <a href="https://business.facebook.com/messaging/whatsapp/onboard/?app_id={{ $metaAppId }}&config_id=985404944212338&extras=%7B%22version%22%3A%22v4%22%2C%22sessionInfoVersion%22%3A%223%22%2C%22featureType%22%3A%22whatsapp_business_app_onboarding%22%7D&redirect_uri={{ urlencode(route('admin.meta-whatsapp.callback')) }}"
            ...
            <a href="https://www.instagram.com/oauth/authorize?force_reauth=true&client_id=1548404910226347&redirect_uri=https://crm.flut.com.br/auth/meta/callback&response_type=code&scope=instagram_business_basic%2Cinstagram_business_manage_messages%2Cinstagram_business_manage_comments%2Cinstagram_business_content_publish%2Cinstagram_business_manage_insights"
                    style="display:flex; align-items:center; gap:10px; padding:12px 24px; background:linear-gradient(135deg, #833AB4, #E1306C, #F77737); border:none; border-radius:10px; color:white; font-size:13px; font-weight:700; cursor:pointer; transition:all 0.2s; box-shadow:0 2px 12px rgba(225,48,108,0.3); white-space:nowrap; text-decoration:none;"
                    onmouseover="this.style.transform='translateY(-1px)'; this.style.boxShadow='0 4px 16px rgba(225,48,108,0.4)'"
                    onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 2px 12px rgba(225,48,108,0.3)'">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="white">
